...
<body>
	<table>
		<caption>1:1문의 관리자로 임시보기</caption>
		<tr>
			<th>답변받을이메일</th>
			<th>문의등록시간</th>
			<th>문의분류</th>
		...
		<c:forEach var="list" items="${claimAdminList }">
			<tr>
			...
			<tr id="content${list.cl_num }" style="display:none;">
				<td colspan="4">
					${list.cl_content }
					<input type="button" value="문의답변하기"
					onclick="location.href='${conPath}/claimAdminAnswerView.do?cl_email=${list.cl_email}&cl_num=${list.cl_num}&cl_content=${list.cl_content}'"/>
				</td>
			</tr>
			<script>
				$(function(){ 
